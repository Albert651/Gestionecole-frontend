...
export default function Inscription() {
  ...
  async function soumettre() {
    ...
    try {
      const data = await inscription(form)
      connecter(data) // on connecte directement apres l'inscription
      navigate('/')
    } catch (e) {
      setErreur(e.message)
    } finally {
  ...
  return (
    <div className="mx-auto max-w-md px-6 py-16 animate-fadeUp">
      <p className="text-or font-semibold tracking-widest uppercase text-xs">Rejoignez-nous</p>
      <h1 className="mt-3 font-display text-4xl font-semibold leading-tight">Créer un compte</h1>
      ...
      <p className="mt-6 text-sm text-ardoise">
        Déjà un compte ?{' '}
        <Link to="/connexion" className="text-or font-semibold hover:underline">Se connecter</Link>
      </p>
    </div>
  )
}